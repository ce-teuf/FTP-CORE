...
pub fn flux_func_stock_var(ftp_result: &mut FtpResult, rownum: usize, colnum: usize) {
    ...
    if let Some(varstock_amort) = &mut ftp_result.varstock_amort {
        if rownum == 0 {
            ...
            varstock_amort[[rownum, colnum]] =
                varstock_amort[[rownum, 0]] * m_profile[[rownum, colnum]];
        }
    } else {
